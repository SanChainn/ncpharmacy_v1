...
# --- NEW: Custom Mixin for User-Friendly Permission Handling ---
class CustomPermissionRequiredMixin(PermissionRequiredMixin):
    """
    Custom permission mixin that shows a message and redirects
    instead of raising a 403 Forbidden error.
    """
    def handle_no_permission(self):
        messages.error(self.request, "You do not have permission to perform this action. 🚫")
        # Redirects to the previous page or to the medicine list as a fallback
        return redirect(self.request.META.get('HTTP_REFERER', reverse_lazy('medicine_list')))
...
# MODIFIED: Using the new custom mixin
class MedicineCreateView(CustomPermissionRequiredMixin, CreateView):
...
# MODIFIED: Using the new custom mixin
class MedicineUpdateView(CustomPermissionRequiredMixin, UpdateView):
...
# MODIFIED: Using the new custom mixin
class MedicineDeleteView(CustomPermissionRequiredMixin, DeleteView):
...
# MODIFIED: Using the new custom mixin
class ThresholdUpdateView(CustomPermissionRequiredMixin, UpdateView):
